...
@njit
def generate_samples_asynch(h, J, delta_t, gamma, Nsteps, seed=0):
    """
    -----------------------------------------------------------------
    Arguments:  - h, numpy array of shape Nvar
                - J, numpy array of shape Nvar x Nvar
...
def NADAM_reconstruct(spins, new_h, new_J, cov, dot_cov, delta_t, Nepochs = 500, LAMBDA = 0.01,
    ...
    # Early stopping ------------------------
    max_Delta_J = np.zeros(25)
    max_Delta_h = np.zeros(25)
    delta_L = np.zeros(25)
    L_list = []
    # ---------------------------------------
    ...
    for t in np.arange(1, Nepochs+1):
        ...
        if len(L_list)>=3:
            delta_L= abs(L_list[-1] - L_list[-3])
            if delta_L<= 0.0001:
                print("early stopping implemented at epoch: ", t)
                break
            
            
#         # Early stopping --------------------------------------------------------------------------
#         if t < 25:
#             max_Delta_J[t] = np.round(np.max(np.abs(new_J - old_J)), 4)
#             max_Delta_h[t] = np.round(np.max(np.abs(new_h - old_h)),4)
#         if t >= 25:
#             max_Delta_J = np.delete(max_Delta_J, 0)
#             max_Delta_J = np.append(max_Delta_J, np.round(np.max(np.abs(new_J - old_J)), 4))
            
#             max_Delta_h = np.delete(max_Delta_h, 0)
#             max_Delta_h = np.append(max_Delta_h, np.round(np.max(np.abs(new_h - old_h)), 4))
            
#             # Check for EARLY STOPPING
#             if (len(np.nonzero(max_Delta_J)[0])==0) and (len(np.nonzero(max_Delta_h)[0])==0):
#                 print("early stopping implemented at epoch: ", t)
#                 break
